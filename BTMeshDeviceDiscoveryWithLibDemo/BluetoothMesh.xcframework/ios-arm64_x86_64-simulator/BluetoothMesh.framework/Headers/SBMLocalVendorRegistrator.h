...
/**
 ...
 @return Boolean value that determines whether model is registered.
 */
- (BOOL)isRegistered;
...
/**
 ...
 Typically registration should be done one time for one SBMLocalVendorModel.
 
 ------
 
 Use SBMControlElement:setStatus:successCallback:errorCallback: to send value to vendor model
 and if vendor model supports reply to a message, reply will come through SBMLocalVendorSettingsMessageHandler
